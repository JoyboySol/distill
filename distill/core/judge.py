...
def _normalize_message_list(raw_messages: Any) -> List[Dict[str, Any]]:
...
def _last_message_text(messages: List[Dict[str, Any]], role: str) -> Optional[str]:
    for message in reversed(messages):
...
def _extract_mcq_choice_letter(text: Optional[str]) -> Optional[str]:
...
def try_mcq_boxed_reference(row_data: Dict[str, Any]) -> Optional[str]:
    messages = _normalize_message_list(row_data.get("messages"))
    if not messages:
        return None
    user_text = _last_message_text(messages, "user")
    if not _looks_like_choice_prompt(user_text):
        return None
    assistant_reference = _last_message_text(messages, "assistant")
...
def _judge_output_impl(row_data: Dict[str, Any],
    ...
    if mcq_reference is not None:
        prediction = _extract_mcq_choice_letter(content)
