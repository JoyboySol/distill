...
def _check_constraint(spec: Dict[str, Any], answer: str) -> Tuple[bool, Dict[str, Any]]:
    ...
    if kind == "response_language":
        ...
        if expected != "english":
            return False, {"expected_language": expected}
        ok = _is_english_text(answer)
        return ok, {"expected_language": expected}
    ...
    if kind == "choose_one_option":
        normalized = answer.lower()
        matches = [
            option for option in spec["values"] if option.lower() in normalized
        ]
